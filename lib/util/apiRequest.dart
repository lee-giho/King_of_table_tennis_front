...
Future<http.Response> apiRequest(Future<http.Response> Function() request, BuildContext context) async {
  ...
  if (response1.statusCode >= 200 && response1.statusCode < 300) { // 첫 번째 요청의 statusCode가 200으로 성공하면 응답 반환
    return response1;

  } else { // 첫 번째 요청의 statusCode가 200이 아니면
    final refreshAccessTokenResponse = await refreshAccessToken(); // accessToken 재발급 요청

    if (refreshAccessTokenResponse.statusCode >= 200 && refreshAccessTokenResponse.statusCode < 300) { // accessToken 재발급이 성공하면
      ...
      if (response2.statusCode >= 200 && response2.statusCode < 300) { // 두 번째 요청의 statusCode가 200으로 성공하면 응답 반환
        return response2;
